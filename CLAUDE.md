...
- **This repository is public.** No domain names, account names or client names in code,
  docs or commit messages; the lab server is referred to as `CM01`, its site as `P01`, as the
  README already does.
- Lab site: CM01, site code P01, database `CM_P01`, reporting point and SSRS on the same box
  (`http://cm01/ReportServer`, portal `http://cm01/Reports`).
- Sessions run on CM01 itself, usually elevated, under an account that is **not** a ConfigMgr
...
- Counting reports on the server: SSRS REST API,
  `Invoke-RestMethod 'http://cm01/Reports/api/v2.0/Reports?$top=2000' -UseDefaultCredentials -AllowUnencryptedAuthentication`
